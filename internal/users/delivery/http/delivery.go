...
func RegisterHandlers(mux *httprouter.Router, logger log.Logger, authorizer mw.Authorizer, serv users.Service, m *mw.HttpMetricsMiddleware) {
	del := delivery{serv, logger}

	mux.GET("/users/:id", mw.HandleLogger(mw.ErrorHandler(m.MetricsMiddleware(authorizer(mw.Cors(del.get)), logger), logger), logger))
}
